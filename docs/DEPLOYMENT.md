...
**Секреты генерировать реально.** Плейсхолдеры из `.env.example` проходят проверку длины в схеме — приложение
поднимется с ключом подписи JWT, который лежит в публичном репозитории (`INFRA-01`).
...
- Открытые находки безопасности и отказоустойчивости из `REVIEW-FINDINGS.md` — деплой их не закрывает;
  до боевого трафика стоит разобрать хотя бы `BE-OAUTH-01` и `INFRA-01`.
