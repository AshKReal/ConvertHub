...
Мои рекомендации в скобках; спорить здесь до кода.

1. **Область API-ключа.** (Рекомендую: `POST /v1/convert` + `GET /v1/files` + `GET /v1/files/:id/download` —
   иначе ключ может конвертировать, но не забрать список/результат, и это сломанный API. `PATCH /v1/files/:id`
   (тумблер `save`) остаётся только под сессией — веб-действие, `TECH-SPEC.md` §8.1.) Альтернатива — 012 трогает
   только `/v1/convert`, файловые маршруты по ключу — отдельной задачей.
2. **Минутный лимит `API_RATE_LIMIT` (60/мин) — на пользователя или на ключ.** (Рекомендую на пользователя:
   у одного пользователя может быть до 3 ключей, «на ключ» дало бы 180/мин суммарно.) Альтернатива — на ключ,
   ближе к тому, как это обычно понимают у API-провайдеров.
3. **Новый код ошибки `IDEMPOTENCY_KEY_CONFLICT` (409).** (Рекомендую завести — API-потребителю понятнее, чем
   переиспользованный `SERVICE_OVERLOADED`.) Альтернатива — вернуть `503 SERVICE_OVERLOADED` без нового кода.
4. **Хранение ответа для идемпотентности.** (Рекомендую: байты результата в Redis, TTL 24ч, потолок =
   `MAX_FILE_SIZE_BYTES`, при желании gzip.) Альтернатива — хранить только `{conversionId, storageKey, mime}` и
   до-стримить из `Storage`; отвергается ниже (гостевой/`save:false` файл живёт 1ч < 24ч окна идемпотентности).
5. **Невалидный ключ на `/v1/convert` → жёсткий `401`, не тихий гость.** (Рекомендую — кривой ключ почти всегда
   реальный баг интеграции, разработчик должен его увидеть; протухший JWT в фоновой вкладке — нет, там остаётся
   тихий откат.) Альтернатива — единообразно с JWT: любой сбой auth → гость.
...
**Порядок в конвейере `/v1/convert`:** resolve identity → (idempotency: replay? → выход) → rate-limit `consume`
→ существующая обработка → (idempotency: store) → ответ. Всё до Multer, кроме store.
