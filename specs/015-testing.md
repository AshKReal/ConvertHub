...
- [ ] `TECH-SPEC.md` §3: «Vitest, Testcontainers, Playwright» → «Vitest (юнит + e2e), Playwright (браузерный e2e)»
- [ ] `ARCHITECTURE.md` §11: таблица уровней и абзац «Testcontainers, а не in-memory» → вторая БД на общем контейнере
- [ ] `SPECS.md` реестр: 015 → `✓` (реальное покрытие + Playwright на месте)
- [ ] `tasks.md`: галочки стадии 9 / 015
- [ ] `AI-JOURNAL.md`: §15 (тесты пишет автор реализации — почему приняли, что иначе в проде); `ci.yml` заведён вне очереди коммитом `a074ba0`
