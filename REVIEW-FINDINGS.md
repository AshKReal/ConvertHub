...
Обновлено 2026-09-03, после стадии 10. Открыто **~85** пунктов; регрессионная сеть под них уже есть (015).

1. **Решение до приёмки 018:** `BE-DOCX-01` — валидатор бомбы верит заявленному размеру. Не 🔴 (ущерб ограничен
   `mem_limit`, `internal`-сетью и таймаутом), но это и есть 🔒-содержание 018: принять как известный предел
   или починить потоковой инфляцией.
2. **Эксплуатируемое:** `BE-OAUTH-01` (🔴 pre-hijacking — единственный оставшийся 🔴), `INFRA-01` (слабые
   секреты в закоммиченном compose — ключ подписи JWT лежит в публичном репозитории).
3. **Отказоустойчивость и ресурсы:** `BE-CONV-01/02/03`, `BE-PUB-02/03`, `BE-OAUTH-02/03`, `BE-DOCX-04/05/06`.
4. **Ломают пользовательские сценарии:** `FE-AUTH-01/02`, `FE-CONV-01/02`.
5. **Канон и документация:** `DOC-03` (противоречие про refresh-токены — самое опасное, уводит следующую
   правку auth), `DOC-01`, `DOC-02`, `DOC-08`, дальше остальные `DOC-*`. Дёшево и разблокирует чтение канона.
6. **Приёмка владельцем по спекам** (раздел выше) — включая построчную 🔒 для 002/003/007–012/018.
7. Остальные 🟡 — по ходу приёмки соответствующих спек; 🟢 — когда рядом.
